...
def future_pay(user_password, balance_user):
    ...
    if pass_try(password, user_password):
        ...
        if summ <= balance_user:
            ...
            print(f'Отложенный платеж успешно выполнен на сумму {summ}, отправлен на логи: {login_to_pay}')
...
def recover_data_YES(user_login, transactions, user_data_func):
    try:
        ...
        print("Файл с транзакциями отсутствует или поврежден")
    ...
    print(f'Ваш баланс: {user_data_func['balance']}')
    return user_data_func


def recover_login(user_login, transactions, user_data_func):
    try:
        password = input('Введите пароль: ')
        with open(f'login.passwordhash.txt', encoding='UTF-8') as file_hash:
            for line in file_hash:
                if user_login in line:
                    user_login, password_old = line.split()
                    if pass_try(password, password_old):
                        user_data = recover_data_YES(user_login, transactions, user_data_func)
                        return user_data
                    else:
                        print()
...
if __name__ == "__main__":
    ...
    if Bank_login == 'Войти':
        ...
        user_data = recover_login(login, transactions_data, user_data)
        password = user_data[login]
